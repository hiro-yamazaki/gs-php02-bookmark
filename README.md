...
1. MAMPを起動（Apache / MySQL。MySQLポートは3306＝現在の `funcs.php` の設定。MAMPデフォルトの8889に戻した場合は `funcs.php` も合わせる）
...
- **直接アクセス対策**: insert.php / update.php / delete.php はPOST以外を弾き、未入力・書籍名64文字超はフォームへリダイレクト（クライアントのrequired/maxlengthに頼らない）
